defmodule PubsubGrpc.ConnectionWorker do
  @moduledoc """
  ...
  - Environment-agnostic connection handling (production, emulator, custom)
  ...
  """
  @spec start_link(PoolConfig.t()) :: GenServer.on_start()
  ...
  defp create_connection(config) do
    {host, port, opts} = PoolConfig.get_endpoint(config)
    retry_config = PoolConfig.get_retry_config(config)
